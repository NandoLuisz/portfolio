...
const skills = ["Todos(4)","Java(3)", "C(1)", "Typescript(3)", "ReactJs(3)", "NextJS(1)", "MySQL(3)", "Spring(3)"]

export default function WorkComp(){

    const [filtered, setFiltered] = useState<string>(skills[0])
    ...
    return (
        <section id="work" className="w-full min-h-[70vh] max-md:min-h-[100vh] flex flex-col items-center text-center gap-7 max-md:gap-4 relative">
            ...
            <p className="text-zinc-800 max-md:text-xs">Verifique meus trabalhos.<br />Se você tiver qualquer pergunta sinta-se livre para pergunntar por mais informações.</p>
            ...
            {data.map((project, index) => (
                <div key={index} className="col-span-1 text-start group mb-10 h-[430px] max-md:h-[340px] flex flex-col gap-3">
                    <div className="h-[200px] bg-zinc-200 rounded-xl flex items-center justify-center p-2 cursor-pointer"
                         onClick={() => openModal(index)}>
                        <img src={project.images_preview[0]} 
                             alt={project.images_preview[0]} 
                             className="w-full"
                            />
                    </div>
